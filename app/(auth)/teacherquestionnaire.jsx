...
export default function teacherQuestionnaire() {
    ...
    const [teacherData, setTeacherData] = useState({
        questionnaire: {
            ...
            least_favorite_subjects: selectedSecondarySubject,
            study_challenges: "",
            teaching_specialization: selectedChallenge,
            secondary_subject: selectedSecondarySubject,
            competitive_exam_ready: false,
            support_needed: ["Modern teaching tools", "Curriculum planning"],
            extracurricular_activities: [],
            educational_goals: "To mentor students for competitive exams like JEE and NEET",
        },
        subscription: {
    ...
    const handleNext = () => {
        if (currentStep === 1) {
            ...
            setCurrentStep(currentStep + 1);
        } else if (currentStep === 4 && selectedSubject) {
            setCurrentStep(currentStep + 1);
        } else if (currentStep === 5 && selectedChallenge) {
            setCurrentStep(currentStep + 1);
            ...
            setCurrentStep(currentStep + 1);
        } else if (currentStep === 7 && selectedSupportType) {
            setCurrentStep(currentStep + 1);
        } else if (currentStep === 8) {
            if (selectedDigitalTools === 'Yes (Please Specify)' && !digitalToolsDetails) {
                alert("Please specify the digital tools.");
                return;
            }
            setCurrentStep(currentStep + 1);
    ...
    const renderStepContent = () => {
        switch (currentStep) {
            ...
            return (
              <View style={styles.formContainer}>
                ...
                <ScrollView>
                  ...
                  {/* Show the input box when "Other (Please Specify)" is selected */}
                  {selectedGoal === 'Other (Please Specify)' && (
                    <TextInput
                      style={styles.textInput}
                      placeholder="Please specify..."
                      placeholderTextColor="#888"
                      onChangeText={(text) => setOtherPrimaryGoal(text)} // Update "Others" input state
                      value={otherPrimaryGoal} // Display "Others" input value
                    />
                  )}
                </ScrollView>
          
                <View style={styles.navButtons}>
                  <TouchableOpacity style={styles.navButtonPrevious} onPress={() => router.push('(auth)/Questionnaire')}>
            ...
            case 2: // Step 2: Select School and Class
                return (
                    <View style={styles.formContainer}>
                        <Text style={styles.questionText}>What classes do you teach?</Text>
                        {/* School Selection */}
                        <ScrollView>
                ...
                return (
                  <View style={styles.formContainer}>
                    <Text style={styles.questionText}>
                      Which educational boards do you teach under or seek support for?
                    </Text>
                    <ScrollView>
                      <View style={styles.optionsGrid}>
            ...
            case 4: // Step 4: Least Favorite Subject
                return (
                    <View style={styles.formContainer}>
                        <Text style={styles.questionText}>What is your least favorite subject?</Text>
                        <ScrollView>
                            <View style={styles.optionsGrid}>
                                {[
                                    'Mathematics',
            ...
            case 5: // Step 5: Challenges in Teaching
                return (
                    <View style={styles.formContainer}>
                        <Text style={styles.questionText}>What challenges do you face in teaching?</Text>
                        <ScrollView>
                            <View style={styles.optionsGrid}>
                                {[
                ...
                case 6: // Step 6: Secondary Subject
                return (
                  <View style={styles.formContainer}>
                    <Text style={styles.questionText}>What is your secondary subject?</Text>
                    <ScrollView>
                      <View style={styles.optionsGrid}>
                        {[
                          'Mathematics',
            ...
            case 7: // Step 7: Type of Support
                return (
                    <View style={styles.formContainer}>
                        <Text style={styles.questionText}>Which type of support do you need?</Text>
                        <ScrollView>
                            <View style={styles.optionsGrid}>
                                {[
                                ...
                                ].map((supportType, index) => (
                                    <TouchableOpacity
                                        key={index}
                                        style={styles.option}
                                        onPress={() => setSelectedSupportType(supportType)}
                                    >
                                        <View style={styles.radioButton}>
                                        ...
                                        <Text style={styles.optionText}>{supportType}</Text>
                                    </TouchableOpacity>
                                ))}
                            </View>
                        </ScrollView>
            ...
            case 8: // Step 8: Digital Tools Usage
                return (
                    <View style={styles.formContainer}>
                        <Text style={styles.questionText}>
                            Do you use any digital tools for teaching?
                        </Text>
                        <ScrollView>
                            <View style={styles.optionsGrid}>
                                {['Yes (Please Specify)', 'No'].map((option, index) => (
                                    <TouchableOpacity
                                        key={index}
                                        style={styles.option}
                        ...
                        {selectedDigitalTools === 'Yes (Please Specify)' && (
                            <View style={styles.inputContainer}>
                                <TextInput
                                    style={styles.input}
                                    placeholder="Please specify the digital tools..."
                                    placeholderTextColor="#ccc"
                                    value={digitalToolsDetails}
                                    onChangeText={setDigitalToolsDetails}
                                />
                            </View>
                        )}
                        <View style={styles.navButtons}>
            ...
            case 9: // Step 9: Digital Tools Usage
                return (
                    <View style={styles.formContainer}>
                        <Text style={styles.questionText}>
                            Would you like support for career growth in teaching?
                        </Text>
                        <ScrollView>
                            <View style={styles.optionsGrid}>
                                {['Yes (Please Specify)', 'No'].map((option, index) => (
                        ...
                        {/* Input Field for "Yes" Option */}
                        {selectedSupport === 'Yes (Please Specify)' && ( // Updated here
                            <View style={styles.inputContainer}>
                                <TextInput
                                    style={styles.input}
  ...
  <View style={styles.progressGroup}>
    {/* Show steps 1-5 if currentStep is between 1 and 5 */}
    {currentStep <= 5 &&
    ...
    {/* Show steps 6-10 if currentStep is between 6 and 10 */}
    {currentStep > 5 &&
      [6, 7, 8, 9, 10].map((step, index) => {
        const isCompleted = currentStep >= step;
        return (
          <View key={step} style={styles.progressItem}>
            <View style={[styles.progressCircle, isCompleted && styles.completedCircle]}>
